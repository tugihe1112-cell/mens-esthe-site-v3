...
async function run() {
  ...
  // JEAN ROBINの検索キーワードとロゴURL
  const searchKeywords = ['jean robin', 'ジャンロビン'];
  const logoUrl = 'https://azuetkuzzmshqfbrhqmf.supabase.co/storage/v1/object/public/shop-logos/jean%20robin.png';

  try {
    console.log(`🚀 「JEAN ROBIN (ジャンロビン)」のロゴ設定を開始します...\n`);
    ...
    if (targets.length > 0) {
      ...
      console.log(`   ⚠️ JEAN ROBINが見つかりませんでした。`);
    }
