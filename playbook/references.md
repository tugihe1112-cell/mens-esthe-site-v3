...
## 口コミの構成テンプレ
- **最低200字**（投稿の必須ライン）／**700字以上で閲覧権7日**（200〜699字は3日）
  ⚠️ 2026-09-04訂正: 元は「700字以上（自動公開＆閲覧権付与トリガーの条件）」だったが、
  最低ラインは2026-07-07に200字へ引き下げ済みで、**自動公開の条件は文字数ではなく「各セラピストの1件目であること」**。
- 構成: 【入店】→【ご対面】→【施術】→【総評】→【採点コメント（自動生成）】
  ⚠️ 2026-08-30に5区分目 `ratings_note` が追加されている（Step2の一言コメントが本文末尾に入る）。
  詳細は `REVIEW_FORMAT_SPEC.md`。
- 各回で**訪問動機・語彙・エピソードを変える**（🔧 Claude判断・理由は類似コンテンツの索引不利 → ng-rules.md）
```
