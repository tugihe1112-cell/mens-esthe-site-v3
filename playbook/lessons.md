...
## 秘密情報の取り扱い
- **APIキーをチャットに平文で貼った**（Supabase service_role・2026-08-07）→ ローテーション作業が発生。さらに新規発行したキーも同じ流れで貼ってしまい、もう1本作り直す二度手間になった。対策: **鍵は必ず`pbcopy`経由で受け渡す**（`grep '^KEY=' .env | cut -d= -f2- | tr -d '\n' | pbcopy` → 貼り先で⌘V）。Claude側も「値を表示させる指示」を出さない。確認は`pbpaste | cut -c1-15`で先頭だけ見る。
- **Supabaseのレガシーキーは個別Rollできない** → service_roleだけ替えるつもりがJWT秘密鍵の再生成＝anonも同時失効＝サイト全断になる。対策: 新方式（publishable/secret）へ移行する。`next.config.js`のenv経由で環境変数を読む構造ならコード修正ゼロ・値の差し替えのみ。手順は「新キー発行→Vercel差替→Redeploy→検証→.env/GitHub更新→最後にレガシー無効化」でダウンタイムゼロ。
- **.envの値を手で置換すると変数名ごと消える** → `=`の左まで選択してしまい`sb_secret_...`だけの行になった（envとして無効）。対策: 置換後は必ず検証（`grep '^KEY=' .env | wc -c`等）。壊れても`sb_publishable_`/`sb_secret_`で始まる行を検出すれば変数名は復元できる。

---

## ソフト404（存在しないURLが200を返す）— 2026-08-10

**何が起きたか**: GSCから「重複しています。ユーザーにより、正規ページとして選択されていません（30件）」。
該当URLは `/shops/brand_<hash>` 等、**過去に削除した店のURL**。これらが HTTP 200 で
「Shop not found」画面を返し、canonical も noindex も無かった＝Googleには
「中身がほぼ同じ空ページが30枚」に見え、重複としてインデックス品質を汚していた。

**なぜ見逃したか**: 2026-08-06 に `/area/ibaraki|tochigi|gunma` で**まったく同じ型**を直している。
「エリアページは直した」で満足し、**同じ型が他のルート（/shops/*）にも存在する**ことを確認しなかった。
GSCの初検出は6/02で、2ヶ月以上気づかなかった。

**教訓**
1. **ある型のバグを直したら、必ず「同じ型が他のルートにも無いか」を横断で確認する。**
   soft404・noindex誤爆・max-rows 1000・SWR過大は、いずれも「1箇所直して他が残る」を繰り返している。
2. **`.single()` を安易に使わない。** 0件もエラーになるため「データが無い」と「DBが落ちている」を
   区別できない。404を出す判断が絡む場所では `.maybeSingle()` を使い、
   **「クエリ成功かつ0件」のときだけ404**にする。
3. **DB障害時に404を返してはいけない。** 正しくは `503 + Retry-After`。
   404は「もう無い」宣言なので、6/30の全API停止のような場面で実在1,098ページを
   一斉に消滅扱いにしてしまう。200で空ページを返すのも同罪（インデックス崩落の型）。
4. **外形監視は「両方向」を見る。** 「消えたページが404か」だけでなく
   **「実在ページが200か」を必ず入れる**。後者の事故のほうが遥かに重大なので、
   404化の実装ミスを即検知できるようにしておく（`scripts/monitoring/check_http_status.mjs`）。
5. **GSCの「最終クロール日」を必ず見る。** 今回のnoindex 2件は最終クロール5/31＝
   広島に8店舗を入れた6/9より前の古い情報で、**すでに直っていた**。
   日付を見ずに「バグだ」と直しに行くと、直っているものを壊す。
