 ...
 * Googleが一度取得したものの、URL検査時点では登録していないという状態を示す。
 * ただし、URL検査APIだけでは品質・重複・サイト全体の評価などの原因を単独特定できない。
 * 内部リンク、canonical、最終クロール日、ページ種別ごとの推移と合わせて判断する。
 *
 *   A. 「まだ発見されていない / クロールされていない」 → 発見経路を優先して点検
 *   B. 「クロール済みだがインデックスしない」         → 内容・重複・サイト評価を追加調査
 *   C. 「重複。Googleが別URLを正規と判断」            → canonicalと近似コンテンツを点検
 *
 ...
 *
 * 2026-09-05: 店舗・セラピストの両方を検査する。
 * 店舗ページにも公開口コミ本文をSSRする修正を本番反映済みなので、ページ種別ごとの
 * 登録率と最終クロール日を追い、修正後HTMLをGoogleが再取得した後の変化を比較する。
 */
...
async function main() {
  ...
  // ── ページ種別ごとの登録率 ─────────────────────────────────────
  // 1回の断面では原因を断定せず、修正後の再クロール日と登録率の時系列変化を見る。
  const isIndexed = (r) => /Submitted and indexed|Indexed, not submitted/i.test(r.coverage);
  const groups = [
    ['店舗ページ（公開口コミ本文をSSR）', ok.filter((r) => /\/shops\/[^/]+$/.test(r.url))],
    ['セラピストページ（公開口コミ本文をSSR）', ok.filter((r) => /\/threads\//.test(r.url))],
  ...
  console.log(`
── 読み方 ──────────────────────────────────────────────
「Crawled - currently not indexed」が多い  → 取得済みだが現在は未登録。
   この状態だけで「薄い」「被リンク不足」と原因を断定できない。修正後の再クロールを待ち、
   canonical・重複・独自内容・サイト評価を時系列で切り分ける。 今回: ${crawledNotIndexed}件

「Discovered - currently not indexed」が多い → クロール予算が足りない。
   sitemapと内部リンクを先に確認し、継続する場合にサイト評価も調べる。今回: ${discovered}件

「URL is unknown to Google」が多い          → そもそも発見されていない。
   サイトマップ／内部リンク／公開時期を確認する。              今回: ${unknown}件

「別URLを正規と判断」                        → 系列店どうしの近似コンテンツ。
   canonical設定とページ間の近似度を個別確認する。             今回: ${dupCount}件
