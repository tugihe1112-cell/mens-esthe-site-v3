 ...
 *
 * 原因:
 *   初回登録時に therapistId.replace(/[^\w-]/g, '_') でStorage ファイル名を生成した
 *   → 日本語名が同文字数だと同一ファイル名になり上書き衝突が発生
 *
 * 修正:
 *   KOAKUMA_DATA（name→S3 UUID マッピング）を使い、
 *   全セラピスト（null含む・既設定含む）をUUIDベースファイル名で再アップロード
 *
 * 実行:
 *   node scripts/maintenance/fix_koakuma_photo_collision.mjs --dry-run
 *   node scripts/maintenance/fix_koakuma_photo_collision.mjs
 */
...
// Chrome DOM 抽出データ（name → UUID.ext）
const KOAKUMA_DATA = [
  ...
  ['白雲ふわり', 'fbb47bdd-3863-4823-862a-cd5ea7b6df6f.jpg'],
...
async function uploadImage(imageUrl, fileName) {
  ...
  } catch { process.stdout.write(`[ERR]`); return null; }
}

// ---- メイン処理 ----

console.log('=== 小悪魔スパ東京 写真衝突修正スクリプト ===\n');

// 小悪魔の全店舗取得（website_url に mens-esthe-aroma を含む）
...
if (!shops?.length) {
  console.log('小悪魔スパ東京の店舗が見つかりません');
  process.exit(1);
}
console.log(`対象店舗: ${shops.map(s => `${s.name}(${s.id})`).join(', ')}`);
const shopIds = shops.map(s => s.id);

// 全セラピスト取得（null含む・既設定含む・全員対象）
...
if (!allTherapists?.length) {
  console.log('セラピストが見つかりません');
  process.exit(1);
}

console.log(`\nセラピスト総数: ${allTherapists.length}名`);

// 現在の衝突を確認
const urlCount = {};
for (const t of allTherapists) {
  if (!t.image_url) continue;
  urlCount[t.image_url] = (urlCount[t.image_url] || 0) + 1;
}
const currentCollisions = Object.values(urlCount).filter(c => c > 1).length;
console.log(`現在の衝突URL数: ${currentCollisions}件`);

// マッチング確認
const matched = allTherapists.filter(t => findMatch(t.name));
const notFound = allTherapists.filter(t => !findMatch(t.name));
console.log(`データマッチ: ${matched.length}名 / 未マッチ: ${notFound.length}名`);
if (notFound.length > 0) {
  console.log('未マッチ名前:');
  notFound.forEach(t => console.log(`  ❓ "${t.name}" (shop: ${t.shop_id})`));
}

if (DRY_RUN) {
  console.log('\n[DRY RUN] ここで終了します。実際に実行するには --dry-run を外してください。');
  process.exit(0);
}
...
const uploadedUrls = new Map(); // S3 URL → Storage URL キャッシュ
...
for (const t of allTherapists) {
  ...
  // UUID ベースのファイル名を生成（衝突しない）
  const uuidMatch = imageUrl.match(/([a-f0-9-]{36})\.(\w+)$/i);
  const uuid = uuidMatch?.[1]?.replace(/-/g, '') || Date.now().toString();
  const rawExt = uuidMatch?.[2]?.toLowerCase() || 'jpg';
  const safeExt = rawExt === 'jfif' ? 'jpg' : rawExt === 'jpeg' ? 'jpg' : rawExt === 'JPG' ? 'jpg' : rawExt;
  ...
  const finalUrl = storageUrl ?? imageUrl; // Storage失敗時はS3 URLを直接使用

  // すでに正しいURLが設定済みならスキップ
  if (t.image_url === finalUrl) {
    process.stdout.write('=');
    skipped++;
    continue;
  }
...
console.log(`⏭️  スキップ（未マッチ/変更不要）: ${skipped}名`);
console.log(`❌ 失敗: ${failed}名`);
console.log('\n再度 check_photo_collisions.mjs を実行して衝突がなくなったか確認してください。');
