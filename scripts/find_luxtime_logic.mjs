...
function search(dir) {
  ...
  for (const item of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (item.isDirectory()) search(p);
      ...
      // luxtimeが含まれているファイルを探し出す
      if (text.toLowerCase().includes('luxtime')) {
        const lines = text.split('\n');
        // luxtimeの文字がある行の前後を広めに抽出して、HTML（JSX）とCSSの構造を見る
        const idx = lines.findIndex(l => l.toLowerCase().includes('luxtime'));
        if (idx !== -1) {
          console.log(`\n🎯 【お手本コード発見（luxtime）】: ${p}`);
          console.log('--------------------------------------------------');
...
console.log('🔍 「luxtime」の完璧なロゴ実装コードを探しています...');
search('src');
