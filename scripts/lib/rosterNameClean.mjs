...
const SUFFIXES = [/セラピストの詳細$/, /の詳細$/, /の?(bluesky|twitter|instagram|tiktok|その他sns|sns)$/i, /(セラピスト|セラピ)$/, /さんの(写真|画像|プロフィール)$/];
// 名前の欄に入っていた「タグ・売り文句」（esthe-alice の「清楚系」「未経験」「イチオシ」、milkrepos の客の名前「〜 様」など）。
// 名前そのものと一致したときだけ捨てる（「可愛川 ゆの」のような名字は残す）。末尾が「系」は名前ではない。
const REJECT_EXACT = new Set(['スレンダー', 'グラマー', '巨乳', '爆乳', '美乳', '美脚', '小柄', '高身長', '色白', '健康的', 'ベテラン', '外国人',
...
export function selfTestRosterNameClean() {
  const cases = [
    ...
    ['本日店休日', null], ['可愛川 ゆの', '可愛川 ゆの'], ['西野さん', '西野さん'], ['アバター', null],
