...
async function run() {
  ...
  // Reactが使用する正しい店舗ID
  const shopId = 'tokyo_setagaya_futakotamagawa_mens_esthe_group';
  
  // 過去の会話でお送りいただいた出勤情報（スケジュール）のリンク
  ...
  console.log(`🧹 1. 間違ったメンエスグループのキャストをSupabaseから一度すべて削除します...`);
  await fetch(`${url}/rest/v1/therapists?shop_id=eq.${shopId}`, {
    method: 'DELETE',
    headers: headers
  });

  console.log(`✨ 2. 正しいキャストデータ（写真・プロフィール付き）を登録します...`);

  // 以前お預かりしたメンエスグループの完全な生データ（74名）
  ...
  const lines = menesRaw.trim().split('\n');
  for (const line of lines) {
    ...
    const payload = {
      ...
      raw_data: { age: age || "", size: sizeStr, T: T || "", B: B || "", sns: sns || "" }
    };
  ...
  console.log(`🔗 3. 店舗データ (shops) に紐付け直します。出勤情報（スケジュールURL）も設定します！`);
  ...
  console.log(`👉 ブラウザをリロードして、「メンエスグループ」を確認してください！`);
}
