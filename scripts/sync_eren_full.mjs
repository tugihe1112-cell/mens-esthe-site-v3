...
const shopId = 'tokyo_shibuya_eren';
...
async function main() {
  console.log('⏳ エレン渋谷店 (tokyo_shibuya_eren) の全データを同期中...');
  ...
  await supabase
    ...
    .eq('id', shopId);
  console.log(' ✅ 店舗情報（スケジュール・料金）を更新しました。');

  // 2. キャストデータを一旦クリア（重複防止のため、現在のshop_idに紐づくものを全削除）
  await supabase
    .from('therapists')
    .delete()
    .eq('shop_id', shopId);
  console.log(' ✅ 旧キャストデータをクリーンアップしました。');
  ...
  if (tErr) {
    ...
    console.log(` 🎉 全${insertData.length}名のキャスト写真を登録完了！`);
  }

  console.log('\n🚀 ブラウザをリロードして、エレンの豪華になったページを確認してください！');
}
