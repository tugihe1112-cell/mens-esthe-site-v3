...
// 🌟 今回の超重要ポイント：多店舗展開の定義
// ここに登録したい店舗のIDとエリアをリストアップしておけば、一気に同じデータを流し込めます
const branches = [
  {
    id: 'tokyo_chiyoda_akihabara_rex', // 先ほど見つけた既存の秋葉原店ID（これを正しい名前に復旧）
    area_id: 'tokyo_chiyoda_akihabara',
    name: 'Rex (レックス秋葉原店)'
  },
  {
    id: 'tokyo_toshima_ikebukuro_rex', // 池袋店用の新しい専用ID
...
async function main() {
  try {
    console.log(`\n==============================================`);
    console.log(`🔄 Rex多店舗（秋葉原店・池袋店）の一括登録を開始します...`);

    for (const branch of branches) {
      console.log(`\n▶️ 処理中: ${branch.name}`);
      
      console.log(`⚙️ 店舗データ(shops)を更新します...`);
      const { error: shopUpsertError } = await supabase
        ...
        .upsert({
          ...
          name: branch.name,
          schedule_url: sharedShopData.schedule_url,
          image_url: sharedShopData.image_url,
          price_system: sharedShopData.price_system
        });

      if (shopUpsertError) throw shopUpsertError;

      console.log(`⚙️ セラピストデータ(therapists)を登録します...`);
      const therapistsPayload = therapistsRaw
        .filter(t => !t.rawName.includes('受付時間')) 
        .map(t => {
          const cleanName = cleanseName(t.rawName);
          return {
            id: `${branch.id}_${cleanName}`, // 例: tokyo_toshima_ikebukuro_rex_三ツ谷まい
            shop_id: branch.id,
            name: cleanName,
            image_url: t.image,
      ...
      console.log(`✅ ${branch.name} のセラピスト ${therapistsPayload.length}件の登録完了`);
    }

    console.log(`\n🎉🎉 秋葉原店・池袋店のデータ一括登録・復旧が完了しました！ 🎉🎉`);
