...
async function run() {
  ...
  const yuruShopId = 'tokyo_shinagawa_gotanda_yuru_spa';
  ...
  const menesShopId = 'tokyo_setagaya_futakotamagawa_mens_esthe_group';
  ...
  console.log(`🚀 完璧な生データからSupabaseへ直接写真をインジェクトします...`);
  ...
  yuruRaw.trim().split('\n').forEach(line => {
    ...
    allCasts.push({
      id: `${yuruShopId}_${name.replace(/[\s　]+/g, '')}`,
      shop_id: yuruShopId,
      name: name.replace(/[\s　]+/g, ''),
      image_url: img ? `https://yuru-spa.com/gotanda/therapist_img/${img}` : "",
      raw_data: { age: age || "", size: sizeStr, sns: sns || "" }
    });
  });
  ...
  menesRaw.trim().split('\n').forEach(line => {
    ...
    const sizeStr = T ? `T.${T} / B.${B || '-'}(-) / W.- / H.-` : "";
    allCasts.push({
      id: `${menesShopId}_${name.replace(/[\s　]+/g, '')}`,
      shop_id: menesShopId,
      name: name.replace(/[\s　]+/g, ''),
      image_url: img ? `https://www.futakotamagawa-mens-esthe.com/wp-content/uploads/2026/04/${img}` : "",
      raw_data: { age: age || "", size: sizeStr, sns: sns || "" }
    });
  });
  ...
  for (const cast of allCasts) {
    ...
    if (res.ok) {
      ...
      console.error(`❌ ${cast.name} エラー:`, await res.text());
      errorCount++;
    }
  }

  console.log(`\n🎉 写真・サイズの完全上書きが完了しました！（成功: ${successCount}名 / 失敗: ${errorCount}名）`);
  console.log(`👉 ブラウザをそのままリロードしてください。（npm run sync は不要です！）`);
}
