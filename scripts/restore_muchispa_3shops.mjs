...
const LOGO_URL = 'https://azuetkuzzmshqfbrhqmf.supabase.co/storage/v1/object/public/shop-logos/muchispa.png';

// 3店舗の設定
const SHOPS = {
  '亀戸': { // デフォルト（本店）
    id: 'tokyo_koto_kameido_muchispa_room',
    name: 'むちすぱルーム 亀戸',
    area_id: 'tokyo_koto_kameido',
    raw_data_base: { prefecture: '東京都', city: '江東区', area: '亀戸' }
  },
  '北千住': {
...
async function main() {
  console.log('🚀 「むちすぱルーム」の全3店舗（亀戸・北千住・南浦和）とセラピスト登録を開始します...\n');

  try {
    // 1. 店舗データの登録
    console.log('🏪 3店舗分の店舗データを登録中...');
    ...
    items.each((_, el) => {
      ...
      // ✨ 所属店舗のIDを決定（記載がなければ本店である「亀戸」に設定）
      let targetShopId = SHOPS['亀戸'].id;
    ...
    console.log(`✅ ${newTherapists.length} 名のセラピストデータを抽出・振り分けました。（無印は亀戸店へ）`);
    ...
    console.log(`\n🎉 登録完了！「むちすぱルーム」の亀戸、北千住、南浦和の3店舗にセラピストが復元・登録されました。`);
