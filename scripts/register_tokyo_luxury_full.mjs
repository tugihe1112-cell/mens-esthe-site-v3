...
const SHOP_ID = 'tokyo_taito_ueno_tokyo_luxury'; // 既存の枠を想定
...
async function main() {
  ...
  try {
    // 1. 店舗の登録（既存の tokyo_taito_ueno_tokyo_luxury を利用・上書き）
    console.log('🏪 店舗データを更新中...');
    
    const SHOP_DATA = {
      id: SHOP_ID,
      name: '東京ラグジュアリー（旧ビジョスパ）',
      area_id: 'tokyo_taito_ueno', // 上野エリア
      group_id: GROUP_ID, // クチコミ吸収用
      schedule_url: 'https://tokyo-luxury.xyz/schedule/',
      website_url: 'https://tokyo-luxury.xyz/',
      business_hours: '11:00〜05:00', // 仮置き
      price_system: '80分 18,000円～', // 画像から反映
      image_url: 'https://placehold.jp/9b59b6/ffffff/400x300.png?text=%E6%9D%B1%E4%BA%AC%E3%83%A9%E3%82%B0%E3%82%B8%E3%83%A5%E3%82%A2%E3%83%AA%E3%83%BC', // 仮画像
      raw_data: {
        prefecture: '東京都',
        city: '台東区',
        area: '上野',
    ...
    items.each((_, el) => {
      ...
      if (!nameText || nameText.includes('割')) return; // 「新人割」「フリー割」などを除外
      ...
      // サイズ情報を抽出 (例: <p>160cm</p> <p>B.89(E)W.55H.86</p>)
    ...
    console.log(`\n🎉 登録完了！「東京ラグジュアリー」に店舗と ${newTherapists.length}名のセラピストがフル登録されました。`);
