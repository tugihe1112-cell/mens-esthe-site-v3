...
async function run() {
  ...
  // トウキョウパニックの検索キーワードとロゴURL
  const shopDef = { 
    searchKeywords: ['tokyo panic', 'トウキョウパニック', '東京パニック'], 
    logo: 'https://azuetkuzzmshqfbrhqmf.supabase.co/storage/v1/object/public/shop-logos/tokyopanic.png' 
  };

  try {
    console.log(`🚀 「Tokyo Panic」のロゴ一括設定を開始します（全系列店対応版）...\n`);
    ...
    if (targets.length > 0) {
      for (const target of targets) {
        ...
        const patchRes = await fetch(`${url}/rest/v1/shops?id=eq.${target.id}`, {
          method: 'PATCH',
          headers: headers,
          body: JSON.stringify({ image_url: shopDef.logo })
        });
      ...
      console.log(`   ⚠️ 店舗が見つかりません: ${shopDef.searchKeywords[0]}`);
    }
