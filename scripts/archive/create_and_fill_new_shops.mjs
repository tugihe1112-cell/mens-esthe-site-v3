...
async function run() {
  try {
    console.log(`📂 ローカル環境に新しい店舗とキャストのデータを生成します...`);
    ...
    // 1. ゆるスパ 五反田店 の作成
    // ==========================================
    const yuruFilePath = 'src/data/tokyo/shinagawa/gotanda/yuru_spa.json';
    ...
    const yuruShopObject = {
      ...
      image: "https://yuru-spa.com/gotanda/img/logo.png", // 仮のロゴURL
      therapists: yuruCasts
    };

    fs.writeFileSync(yuruFilePath, `export default ${JSON.stringify(yuruShopObject, null, 2)};\n`, 'utf-8');
    console.log(`   ✅ ゆるスパ 五反田店 のデータを作成しました: ${yuruFilePath}`);


    // ==========================================
    // 2. メンエスグループ 二子玉川 の作成
    // ==========================================
    const menesFilePath = 'src/data/tokyo/setagaya/futakotamagawa/menes_group.json';
    ...
    const menesCastsFormatted = menesData.trim().split('\n').map(line => {
      ...
      return { id: `menes_group_${name}`, name, age: age || "", T: T || "", B: B || "", tags: tags ? tags.split('/') : [], image: img ? `https://www.futakotamagawa-mens-esthe.com/wp-content/uploads/2026/04/${img}` : "", sns: sns || "" };
    });
    ...
    fs.writeFileSync(menesFilePath, `export default ${JSON.stringify(menesShopObject, null, 2)};\n`, 'utf-8');
    console.log(`   ✅ メンエスグループ 二子玉川 のデータを作成しました: ${menesFilePath}`);

    console.log(`\n🎊 ローカル環境へのデータ生成が完了しました！`);
    console.log(`👉 最後に、いつものデータ同期スクリプトを実行してSupabaseへ反映させてください。これで完璧に写真と名前が表示されます！`);
