...
async function run() {
  ...
  const aromaData = {
    website_url: "https://tokyoaroma.jp/",
  ...
  // 2. 東京メンズエステ のデータ
  const menesData = {
    website_url: "https://tokyo-menes.com/",
  ...
  try {
    ...
    if (targetAromaShops.length > 0) {
      for (const shop of targetAromaShops) {
        ...
        // 店舗情報の更新 (website_url を使用)
        const patchRes = await fetch(`${url}/rest/v1/shops?id=eq.${shop.id}`, {
          method: 'PATCH',
          headers: headers,
          body: JSON.stringify({ 
            website_url: aromaData.website_url, // ここを修正！
        ...
        if (patchRes.ok) {
          console.log(`   ✅ ホームページ・スケジュール・料金システム更新完了`);
        } else {
    ...
    if (targetMenesShops.length > 0) {
      for (const shop of targetMenesShops) {
        ...
        const patchRes = await fetch(`${url}/rest/v1/shops?id=eq.${shop.id}`, {
          ...
          headers: headers,
          body: JSON.stringify({ 
            website_url: menesData.website_url, // ここを修正！
