...
async function run() {
  ...
  try {
    console.log(`🧹 1. 誤って直接DBに入れてしまったバグデータを削除（ロールバック）します...`);
    const shopIds = ['tokyo_shinagawa_gotanda_yuru_spa', 'tokyo_setagaya_futakotamagawa_menes_group'];
    
    // Supabaseから該当店舗のキャストを削除
    const deleteRes = await fetch(`${url}/rest/v1/therapists?shop_id=in.(${shopIds.join(',')})`, {
      method: 'DELETE',
      headers: headers
    });
    ...
    if (deleteRes.ok) {
      ...
      console.log(`   ⚠️ 削除スキップ（または既に削除済み）`);
    }

    console.log(`\n📂 2. ローカルのデータファイルを検索して正しく更新します...`);

    // ローカルのデータファイルを再帰的に探す関数 (backupフォルダは除外)
    function findFile(dir, keyword) {
      ...
      list.forEach(file => {
        ...
        if (stat && stat.isDirectory() && !filePath.includes('backup')) {
          results = results.concat(findFile(filePath, keyword));
        } else if (filePath.includes(keyword) && filePath.endsWith('.json') && !filePath.includes('backup')) {
    ...
    const yuruFiles = findFile('src/data', 'yuru_spa');
    const menesFiles = findFile('src/data', 'futakotamagawa'); 
    ...
    const yuruCasts = yuruData.trim().split('\n').map(line => {
    ...
    // export default を考慮してファイルを書き換える関数
    function processJsonFile(filePath, newTherapists) {
      let rawContent = fs.readFileSync(filePath, 'utf-8');
      let hasExportDefault = false;
      let jsonStr = rawContent;

      if (rawContent.trim().startsWith('export default')) {
        hasExportDefault = true;
        jsonStr = rawContent.replace(/export\s+default\s+/, '').trim();
        if (jsonStr.endsWith(';')) jsonStr = jsonStr.slice(0, -1);
      }

      let data = JSON.parse(jsonStr);
      data.therapists = newTherapists;
    ...
    if (yuruFiles.length > 0) {
      processJsonFile(yuruFiles[0], yuruCasts);
    } else {
      console.log(`   ⚠️ ゆるスパのJSONが見つかりませんでした。`);
    }

    if (menesFiles.length > 0) {
      processJsonFile(menesFiles[0], menesCastsFormatted);
    } else {
      console.log(`   ⚠️ メンエスグループのJSONが見つかりませんでした。`);
    }

    console.log(`\n🎊 ローカルファイルの修正が完了しました！`);
    console.log(`👉 次に、いつもの同期スクリプト（node src/scripts/syncDb.js や node sync_data.js など）を実行して、Supabaseへ反映させてください！`);
