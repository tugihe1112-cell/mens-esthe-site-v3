...
async function run() {
  ...
  try {
    console.log(`🔍 データベースから「小悪魔スパトウキョウ」を検索し、更新を実行します...\n`);
    ...
    const targetShops = allShops.filter(shop => {
      ...
      // 「小悪魔」または「koakuma」が含まれていれば対象とする
      return n.includes('小悪魔') || n.includes('koakuma');
    });
    ...
    if (targetShops.length > 0) {
      for (const shop of targetShops) {
        ...
        // 1. スケジュールURL、料金システムを更新
        const patchRes = await fetch(`${url}/rest/v1/shops?id=eq.${shop.id}`, {
          method: 'PATCH',
          headers: headers,
          body: JSON.stringify({ 
            schedule_url: koakumaData.schedule_url,
            price_system: koakumaData.price_system
          })
        });
        ...
        // 重複を除去
        const uniqueCasts = Array.from(new Map(koakumaData.casts.map(c => [c.name, c])).values());
        ...
        for (const cast of uniqueCasts) {
          ...
          // お知らせバナーなどを弾く
          if (ignoreNames.some(ignore => cleanName.includes(ignore.replace(/[\s　]+/g, '')))) {
            skipCount++;
            continue;
          }
        ...
        console.log(`  🎉 キャスト設定完了（新規: ${insertCount}名 / 画像更新: ${updateCount}名 / バナー除外: ${skipCount}件）\n`);
      }
      console.log(`🎊 「小悪魔スパトウキョウ」の更新が完了しました！ブラウザをリロードして確認してください！`);
    } else {
      console.log("⚠️ 「小悪魔」を含む店舗が見つかりませんでした。");
    }
