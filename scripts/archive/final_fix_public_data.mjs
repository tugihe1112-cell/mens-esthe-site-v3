...
async function run() {
  try {
    console.log(`📂 真のマスターデータ (public/data/) に店舗とキャストのデータを生成します...`);
    ...
    const menesShopObject = {
      ...
      city: "世田谷区",
      area: "二子玉川",
      websiteUrl: "https://www.futakotamagawa-mens-esthe.com/",
      scheduleUrl: "https://www.futakotamagawa-mens-esthe.com/schedule/",
      price_system: "70分 14,000円 / 90分 18,000円 / 110分 22,000円",
      image: "https://www.futakotamagawa-mens-esthe.com/wp-content/uploads/2023/06/menesgroup-logo-ww.png",
      therapists: menesCastsFormatted
    };

    fs.writeFileSync(menesFilePath, JSON.stringify(menesShopObject, null, 2), 'utf-8');
    console.log(`   ✅ メンエスグループ 二子玉川 のマスターデータを作成しました: ${menesFilePath}`);

    console.log(`\n🎊 public/data へのデータ作成が完了しました！`);
    console.log(`👉 最後にターミナルで npm run sync を実行してください。これで全てが完璧に表示されます！`);
