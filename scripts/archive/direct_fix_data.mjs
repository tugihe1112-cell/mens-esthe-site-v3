...
async function run() {
  try {
    const therapistsPath = 'src/data/therapists.json';
    const shopsPath = 'src/data/shops.json';

    console.log("🛠  ローカルマスターデータを直接修復・更新します...");

    // 1. 各データの読み込み
    let therapists = JSON.parse(fs.readFileSync(therapistsPath, 'utf-8'));
    let shops = JSON.parse(fs.readFileSync(shopsPath, 'utf-8'));
    ...
    const yuruTherapistIds = [];
    yuruRaw.trim().split('\n').forEach(line => {
      const [name, age, T, tags, img, sns] = line.split(',');
      const tid = `${yuruId}_${name}`; // 紐付け用ID
      yuruTherapistIds.push(tid);
      
      // therapists.json への追加・更新
    ...
    // 4. メンエスグループデータの処理
    const menesId = "tokyo_setagaya_futakotamagawa_mens_esthe_group"; // 正しいIDを使用！
    const menesTherapistIds = [];
    menesRaw.trim().split('\n').forEach(line => {
      const [name, age, T, B, tags, img, sns] = line.split(',');
      const tid = `${menesId}_${name}`; // 紐付け用ID
      menesTherapistIds.push(tid);
    ...
    // 5. shops.json 内の店舗にIDリストをセット
    const yuruIndex = shops.findIndex(s => s.id === yuruId);
    if (yuruIndex !== -1) {
      shops[yuruIndex].therapists = yuruTherapistIds;
      console.log(`✅ ゆるスパ五反田店 (${yuruId}) に ${yuruTherapistIds.length} 名のキャストを紐付けました。`);
    } else {
      console.error(`❌ エラー: shops.json にゆるスパ (${yuruId}) が見つかりません。`);
    }

    const menesIndex = shops.findIndex(s => s.id === menesId);
    if (menesIndex !== -1) {
      shops[menesIndex].therapists = menesTherapistIds;
      console.log(`✅ メンエスグループ (${menesId}) に ${menesTherapistIds.length} 名のキャストを紐付けました。`);
    } else {
      console.error(`❌ エラー: shops.json にメンエスグループ (${menesId}) が見つかりません。`);
    }

    // 6. データを保存
    fs.writeFileSync(therapistsPath, JSON.stringify(therapists, null, 2));
    fs.writeFileSync(shopsPath, JSON.stringify(shops, null, 2));

    console.log("\n🎊 完璧な形でのデータ修復が完了しました！");
    console.log("👉 画面（ブラウザ）をリロードして確認してください。");
    console.log("※これでも表示されない場合のみ、最後に [ npm run dev ] などでサーバーを再起動してください。");
