...
async function run() {
  ...
  const targetKeyword = "FioreSpa"; // または "フィオーレスパ"
  
  // 📝 3枚目の写真の料金プランをここに入力してください！
...
〇〇min: 〇〇,000円
  ...
  const updateData = {
    schedule_url: "https://www.fiorespa.net/schedule/",
  ...
  try {
    ...
    // "FioreSpa" または "フィオーレスパ" を含む店舗を検索
    const res = await fetch(`${url}/rest/v1/shops?or=(name.ilike.*FioreSpa*,name.ilike.*フィオーレスパ*)&select=id,name`, { headers });
