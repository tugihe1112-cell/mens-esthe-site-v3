...
//    ここはオーナーが明示的に確認した要件なので、実データで固定する。
{
  ...
  //    実データ:「Aroma ELLA武蔵小杉」「doigt de fee (ドゥワドフェ)溝の口」「武蔵小杉 ROYCE (ロイス)」
  {
    ...
    const cases = [
      ...
      // ⭐ 地名まで含めて店名の店（2026-09-23 実データ）。所在地の修正でエリアに「辻堂」が入り、
      //    「メンズエステ辻堂」が「メンズエステ」と表示された。残りが業種の一般名詞だけになる外し方はしない。
      ['メンズエステ辻堂', { area: ['辻堂'] }, 'メンズエステ辻堂', '⭐外すと一般名詞だけになるなら外さない(実データ)'],
      ['メンズエステ大阪', { prefecture: '大阪府' }, 'メンズエステ大阪', '⭐一般名詞＋都道府県も外さない(実データ)'],
      ['メンズエステ 辻堂', { area: ['辻堂'] }, 'メンズエステ 辻堂', '⭐スペース区切りでも一般名詞だけにしない'],
      ['辻堂 メンズエステ', { area: ['辻堂'] }, '辻堂 メンズエステ', '⭐先頭の地名でも一般名詞だけにしない'],
      ['ＳＰＡ（辻堂）', { area: ['辻堂'] }, 'ＳＰＡ（辻堂）', '⭐全角英字・括弧でも一般名詞だけにしない'],
      ["Men's Esthe 辻堂", { area: ['辻堂'] }, "Men's Esthe 辻堂", '⭐英語の一般名詞も同じ扱い'],
      ['アロマモア辻堂', { area: ['辻堂'] }, 'アロマモア', '一般名詞でなければ従来どおり外す'],
    ];
    ...
    check('⭐表示名: 店舗を渡さない経路でも「〇〇店」を外して一般名詞だけにしない', () => {
      const kept = getDisplayName('メンズエステ 辻堂店');
      if (kept !== 'メンズエステ 辻堂店') return `「${kept}」になった（期待「メンズエステ 辻堂店」）`;
      const cut = getDisplayName('アロマモア 渋谷店');
      if (cut !== 'アロマモア') return `従来の支店名除去が止まった: 「${cut}」`;
      return null;
    });
...
//    トップは取得を全部待ってから返すので、それがそのままトップの待ち時間だった。数えた数を持ち回す仕組みを実際に動かして固定する。
{
  ...
  check('トップの件数: getServerSideProps の中で件数だけの問い合わせをしない（手元の数を使う）', () => {
    ...
    // 数えるのは /api/shops-lite?view=counts（CDNに置く）。トップの関数のメモリで数え直すと、返答後に止まって結果が失われる（2026-09-24）。
    if (!/fetch\(`\$\{SITE\}\/api\/shops-lite\?view=counts`/.test(before)) return '件数を /api/shops-lite?view=counts（CDN）から読んでいない';
    const lite = fs.readFileSync(path.join(ROOT, 'api/shops-lite.js'), 'utf-8');
    if (!/req\.query\.view === 'counts'\) return sendSiteCounts\(req, res\)/.test(lite)) return 'api/shops-lite.js が ?view=counts を server/siteCounts.js に渡していない';
    const api = fs.readFileSync(path.join(ROOT, 'server/siteCounts.js'), 'utf-8')
      .replace(/\/\*[\s\S]*?\*\//g, '')
      .replace(/^\s*\/\/.*$/gm, '');
    if (!/from\('shops'\)\.select\('id', \{ count: 'exact', head: true \}\)/.test(api)) return 'server/siteCounts.js が店舗を数えていない';
...
// ── CDNに置くAPI（店舗一覧 api/shops-lite.js・件数 server/siteCounts.js）が本当にCDNに溜まること（2026-09-24 本番実測）──────────
...
for (const apiFile of ['api/shops-lite.js', 'server/siteCounts.js']) check(`${apiFile}: 成功の応答に ETag を付けない（304ばかりでCDNが空のままになるのを防ぐ）`, () => {
