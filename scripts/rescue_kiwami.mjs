...
const WRONG_SHOP_ID = 'tokyo_ota_omori_gokujou';
const CORRECT_SHOP_ID = 'tokyo_toshima_ikebukuro_kiwami';

const shopData = {
  name: '美・セラ極～KIWAMI～',
  schedule_url: 'https://b-s-kiwami.com/schedule',
  price_system: '90分 ¥15,000\n120分 ¥20,000\n150分 ¥25,000\n180分 ¥30,000',
  area_id: 'tokyo_toshima_ikebukuro'
};
...
async function rescue() {
  console.log('🚨 大森店の誤作動レスキュー＆KIWAMIの正しい登録を開始します...\n');
  const now = new Date().toISOString();
  
  try {
    // 【ステップ1】大森店 (WRONG_SHOP_ID) の復旧
    console.log('🧹 1. 大森店に混ざってしまったKIWAMIのキャストを削除します...');
    const kiwamiCleanNames = therapistsRaw.map(t => cleanseName(t.rawName));
    await supabase.from('therapists').delete().eq('shop_id', WRONG_SHOP_ID).in('name', kiwamiCleanNames);

    console.log('👼 2. 大森店で誤って「退店」扱いになった44名のキャストを復活させます...');
    await supabase.from('therapists').update({ is_active: true }).eq('shop_id', WRONG_SHOP_ID);
    console.log('✅ 大森店内のキャストの復旧が完了しました。');

    // 【ステップ2】美・セラ極～KIWAMI～ (CORRECT_SHOP_ID) の新規・正しい登録
    console.log('\n🏗️ 3. 美・セラ極～KIWAMI～ (池袋) を正しいIDで新規作成します...');
    await supabase.from('shops').upsert({
      id: CORRECT_SHOP_ID, // 絶対にブレない専用IDを発行
    ...
    console.log('👩 4. 正しい池袋の枠に、KIWAMIのキャスト46名を登録します...');
    const payload = therapistsRaw.map(t => {
      const clean = cleanseName(t.rawName);
      return {
        id: `${CORRECT_SHOP_ID}_${clean}`,
    ...
    console.log('\n🎉🎊 レスキューと登録がすべて正常に完了しました！ 🎊🎉');
    console.log('ブラウザに戻り「Cmd + Shift + R」でリロードして、池袋に「KIWAMI」が出現しているか確認してください。');
