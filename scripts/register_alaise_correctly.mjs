...
async function main() {
  ...
  try {
    ...
    items.each((_, el) => {
      ...
      const cupMatch = rawSizeText.match(/\(([A-Z])\)/i); // 大文字小文字両方対応
      ...
      let imageUrl = item.find('img').attr('src') || ''; // 'picture img' だと取得漏れがあるため修正
    ...
    if (therapistsToInsert.length === 0) {
        console.log('⚠️ 抽出できたセラピストがいません。HTMLの構造が変わっている可能性があります。');
        return;
    }
    ...
    console.log(`\n🎉 a laiseのセラピスト再登録が完了しました！ブラウザでご確認ください。`);
