/**
 * セレクト (SELECT) セラピスト・ロゴ・スケリンク・料金 一括登録
 * 実行: node insert_select_therapists.mjs
 */
import fs from 'fs';
...
async function run() {
  ...
  const SHOP_ID = 'tokyo_nerima_oizumigakuen_select';
  const SITE_URL = 'https://esthe-select.com';
  const THERAPIST_PAGE = 'https://esthe-select.com/cast/';
  const SCHEDULE_URL = 'https://esthe-select.com/schedule/';
  const WEBSITE_URL = 'https://esthe-select.com';
  const LOGO_URL = 'https://esthe-select.com/wp-content/uploads/2025/09/h-logo.png';
  const STORAGE_BUCKET = 'shop-logos';
  const LOGO_FILE_NAME = 'Select.png';

  const price_system = `60分【限定】: 13,000円
90分: 13,000円
120分: 17,000円`;

  console.log('🚀 セレクト (SELECT) セットアップ開始\n');
  ...
  // Step 2: パース（WordPress, alt="body maintenance SELECT {名前}の画像"）
  ...
  $('img').each((i, el) => {
    ...
    if (!alt.includes('SELECT') || !alt.includes('の画像')) return;

    // "body maintenance SELECT まつだの画像" → "まつだ"
    const name = alt.replace(/^.*SELECT\s*/, '').replace(/の画像.*$/, '').trim();
