/**
 * Cannele (カヌレ) セラピスト・ロゴ・スケリンク・料金 一括登録
 * 実行: node insert_cannele_therapists.mjs
 */
import fs from 'fs';
...
async function run() {
  ...
  const SHOP_ID = 'tokyo_nerima_nerima_cannele';
  const SITE_URL = 'https://cannele-nerima.com';
  const THERAPIST_PAGE = 'https://cannele-nerima.com/staff.php';
  const SCHEDULE_URL = 'https://cannele-nerima.com/schedule.php';
  const WEBSITE_URL = 'https://cannele-nerima.com';
  const LOGO_URL = 'https://cannele-nerima.com/images/logo_side.png';
  const STORAGE_BUCKET = 'shop-logos';
  const LOGO_FILE_NAME = 'Cannele.png';

  const price_system = `お試し50分(うつ伏せのみ): 10,000円
90分: 17,000円
120分: 21,000円
150分: 25,000円
180分: 29,000円`;

  console.log('🚀 Cannele (カヌレ) セットアップ開始\n');
  ...
  // Step 2: パース（images_staff の img から alt=名前）
  ...
  $('img').each((i, el) => {
    ...
    if (!src.includes('images_staff')) return;

    const name = alt.trim();
    if (!name || name.length < 2) return;
    if (/アイコン|icon|new|new!/i.test(name)) return;
