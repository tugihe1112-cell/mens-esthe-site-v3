/**
 * Aroma BANKER セラピスト・ロゴ・スケリンク・料金 一括登録
 * 実行: node insert_banker_therapists.mjs
 */
import fs from 'fs';
...
async function run() {
  ...
  const SHOP_ID = 'tokyo_adachi_takenotsuka_aroma_banker';
  const SITE_URL = 'https://aroma-banker.com';
  const THERAPIST_PAGE = 'https://aroma-banker.com/therapist/';
  const SCHEDULE_URL = 'https://aroma-banker.com/schedule/';
  const WEBSITE_URL = 'https://aroma-banker.com';
  const LOGO_URL = 'https://aroma-banker.com/img/logo.png';
  const STORAGE_BUCKET = 'shop-logos';
  const LOGO_FILE_NAME = 'AromaBanker.png';

  const price_system = `90分: 15,000円
120分: 19,000円
150分: 23,000円
180分: 27,000円`;

  console.log('🚀 Aroma BANKER セットアップ開始\n');
  ...
  const $ = cheerio.load(html, { baseURI: THERAPIST_PAGE });

  console.log('\n👩 Step 2: セラピストデータをパース中...');
  const casts = [];
  const seen = new Set();

  $('.therapistblock').each((i, el) => {
    const imgEl = $(el).find('img').first();
    let src = imgEl.attr('src') || '';

    // NEWアイコン・ロゴは除外
    if (/ic_new|logo|icon/i.test(src)) return;

    // 相対パスを絶対パスに変換
    if (src.startsWith('../')) {
      src = SITE_URL + src.replace('..', '');
    ...
    // alt属性から名前を取得（「アロマバンカーの〇〇」→「〇〇」）
    const alt = imgEl.attr('alt') || '';
    const name = alt.replace(/アロマバンカーの/, '').trim();
