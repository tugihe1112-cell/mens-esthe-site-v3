/**
 * リオラ (旧マリラ) セラピスト登録
 * 実行: node scripts/insert/insert_liora_therapists.mjs
 */
import * as cheerio from 'cheerio';
import fs from 'fs';

const SHOP_ID = 'kanagawa_yokohama_liora';
const SITE = 'https://liora2024.com';
const SCHEDULE_URL = `${SITE}/schedule`;
const LOGO_URL = `${SITE}/theme/mblmewhite01/images/header_logo.png`;
...
async function run() {
  ...
  const res = await fetch(`${SITE}/girl`, { headers: ua });
  ...
  $('div.girls_box, div[class*="girl"]').each((_, el) => {
    ...
    const raw_data = {};
    // 年齢: "(20)" or "20歳"
    const ageM = text.match(/\((\d{2,3})\)/) || text.match(/(\d{2,3})\s*歳/);
    // 身長: "T.160"
    const heightM = text.match(/T[．.]\s*(\d{3})/);
    // カップ: "B.88(D)"
    const cupM = text.match(/B\.\d+\(([A-J])\)/i);
