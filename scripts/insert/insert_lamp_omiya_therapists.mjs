/**
 * らんぷ大宮店 セラピスト・ロゴ・スケリンク・料金 登録
 * 実行: node scripts/insert/insert_lamp_omiya_therapists.mjs
 */
import fs from 'fs';
...
async function run() {
  ...
  const SHOP_ID = 'saitama_omiya_lamp_omiya';
  const SITE_URL = 'https://omiya.senju-lamp.com';
  const THERAPIST_PAGE = 'https://omiya.senju-lamp.com/therapist.html';
  const SCHEDULE_URL = 'https://omiya.senju-lamp.com/schedule.html';
  const WEBSITE_URL = 'https://omiya.senju-lamp.com';
  const LOGO_URL = 'https://omiya.senju-lamp.com/omiya/logo.png';
  const STORAGE_BUCKET = 'shop-logos';
  const LOGO_FILE_NAME = 'LampOmiya.png';

  const price_system = `お試し90分: 10,000円
120分: 14,000円
150分: 18,000円
180分: 21,000円
240分: 28,000円
