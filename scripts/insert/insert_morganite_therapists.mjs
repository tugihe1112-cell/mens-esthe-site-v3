/**
 * MORGANITE (モルガナイト) セラピスト・ロゴ・スケリンク・料金 一括登録
 * 実行: node insert_morganite_therapists.mjs
 */
import fs from 'fs';
...
async function run() {
  ...
  const SHOP_ID = 'tokyo_tachikawa_morganite';
  const SITE_URL = 'https://a-morganite.com';
  const THERAPIST_PAGE = 'https://a-morganite.com/cast/';
  const SCHEDULE_URL = 'https://a-morganite.com/schedule/';
  const WEBSITE_URL = 'https://a-morganite.com';
  const LOGO_URL = 'https://a-morganite.com/wp-content/themes/morgana/images/mo-rogo-smp.png';
  const STORAGE_BUCKET = 'shop-logos';
  const LOGO_FILE_NAME = 'Morganite.png';

  const price_system = `70分: 16,000円
90分: 18,000円
120分: 23,000円`;

  console.log('🚀 MORGANITE セットアップ開始\n');
  ...
  // Step 2: パース（WordPress, wp-content/uploads, alt=名前）
