/**
 * Alice (アリス) セラピスト・ロゴ・スケリンク・料金 一括登録
 * 実行: node insert_alice_therapists.mjs
 */
import fs from 'fs';
...
async function run() {
  ...
  const SHOP_ID = 'tokyo_kita_alice';
  const SITE_URL = 'https://esthe-alice.net';
  const THERAPIST_PAGE = 'https://esthe-alice.net/schedule.php';
  const SCHEDULE_URL = 'https://esthe-alice.net/schedule.php';
  const WEBSITE_URL = 'https://esthe-alice.net';
  const LOGO_URL = 'https://esthe-alice.net/images/logo_b.png';
  const STORAGE_BUCKET = 'shop-logos';
  const LOGO_FILE_NAME = 'Alice.png';

  const price_system = `90分: 22,000円
120分: 27,000円
150分: 32,000円`;

  console.log('🚀 Alice (アリス) セットアップ開始\n');
  ...
  // Step 2: パース（.cast_box li > img.cast_img）
  console.log('\n👩 Step 2: セラピストデータをパース中...');
  const casts = [];
  const seen = new Set();

  $('.cast_box li').each((i, el) => {
    const imgEl = $(el).find('img.cast_img').first();
    const src = imgEl.attr('src') || '';
    const alt = imgEl.attr('alt') || '';

    if (!src || !src.includes('images_staff')) return;
