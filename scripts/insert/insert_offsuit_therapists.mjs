/**
 * オフスート (Offsuit) セラピスト・ロゴ・スケリンク・料金 登録
 * 実行: node scripts/insert/insert_offsuit_therapists.mjs
 */
import fs from 'fs';
...
async function run() {
  ...
  const SHOP_ID = 'saitama_urawa_offsuit';
  const SITE_URL = 'https://offsuit.site';
  const THERAPIST_PAGE = 'https://offsuit.site/therapist';
  const SCHEDULE_URL = 'https://offsuit.site/schedule';
  const WEBSITE_URL = 'https://offsuit.site';
  const LOGO_URL = 'https://offsuit.site/assets/customer/share_logo-4da3a1f62005626f94fcab4a56b7bb8da4fcca5de336111861f42ce4d48f990d.png';
  const STORAGE_BUCKET = 'shop-logos';
  const LOGO_FILE_NAME = 'Offsuit.png';
