/**
 * 武蔵小杉 ROYCE セラピスト登録
 * 実行: node scripts/insert/insert_royce_therapists.mjs
 */
import * as cheerio from 'cheerio';
import fs from 'fs';

const SHOP_ID = 'kanagawa_musashikosugi_royce';
const SITE = 'https://aromaroyce.com';
const SCHEDULE_URL = `${SITE}/schedule.php`;
const LOGO_URL = `${SITE}/images/logo_header.png`;
...
async function run() {
  ...
  const res = await fetch(`${SITE}/staff.php`, { headers: ua });
  ...
  $('li:has(img[src*="images_staff"])').each((_, el) => {
    // 同一li内に重複img(PC/SP両方)があるので最初の1枚だけ
    const imgEl = $(el).find('img[src*="images_staff"]').first();
    if (!imgEl.length) return;

    // 名前: .name クラス or alt から
    let name = $(el).find('.name').first().text().trim();
    if (!name) {
      // alt: "武蔵小杉ロイス在籍のXXXの写真" → 中間を抽出
      const alt = imgEl.attr('alt') || '';
      name = alt.replace(/^.*在籍の/, '').replace(/の写真$/, '').trim();
    }
    ...
    if (imageUrl && !imageUrl.startsWith('http')) imageUrl = `${SITE}${imageUrl}`;

    const text = $(el).text().replace(/\s+/g, ' ').trim();
    const raw_data = {};
    const ageM = text.match(/Age[.\s]*(\d{2,3})/i);
    const heightM = text.match(/Tall[.\s]*(\d{3})/i);
    const cupM = text.match(/Cup[.\s]*([A-J])/i);
