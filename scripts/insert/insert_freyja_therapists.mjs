/**
 * FREYJA (旧ゼニスパ) セラピスト・ロゴ・スケリンク・料金 一括登録
 * 実行: node insert_freyja_therapists.mjs
 */
import fs from 'fs';
...
async function run() {
  ...
  const SHOP_ID = 'tokyo_nerima_nerima_xenispa';
  const SITE_URL = 'https://freyja-esthe.com';
  const THERAPIST_PAGE = 'https://freyja-esthe.com/therapist';
  const SCHEDULE_URL = 'https://freyja-esthe.com/schedule';
  const WEBSITE_URL = 'https://freyja-esthe.com';
  const STORAGE_BUCKET = 'shop-logos';
  const LOGO_FILE_NAME = 'Freyja.png';
  const SHOP_NAME = 'FREYJA';

  const price_system = `70分: 13,000円
80分: 15,000円
90分: 16,000円
100分: 18,000円
120分: 22,000円`;

  console.log('🚀 FREYJA セットアップ開始\n');
  ...
  try {
    ...
    let logoSrc = logoMatch ? 'https://' + logoMatch[0] : null;
