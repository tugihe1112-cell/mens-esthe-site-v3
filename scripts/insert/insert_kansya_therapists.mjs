/**
 * 感謝 旧愛燦燦 (kansya-osaka.com) セラピスト挿入
 * 実行: node scripts/insert/insert_kansya_therapists.mjs
 */
import * as cheerio from 'cheerio';
import fs from 'fs';
import { createClient } from '@supabase/supabase-js';

const SHOP_ID = 'osaka_shinosaka_mokukindo';
const THERAPIST_URL = 'https://kansya-osaka.com/cast/';
...
async function uploadImage(imageUrl, therapistId) {
  try {
    ...
    return publicUrl;
  } catch (e) { console.log(`  画像エラー: ${e.message}`); return null; }
}
...
async function run() {
  ...
  const therapists = [];
  // figureの親要素からセラピスト情報を取得
  $('figure:has(img[src*="wp-content/uploads"])').each((_, el) => {
    const $el = $(el);
    const imgSrc = $el.find('img').first().attr('src') || '';
    if (!imgSrc) return;

    // figcaptionまたは親要素テキストから名前取得
    const figcaption = $el.find('figcaption').text().trim();
    const parentItem = $el.closest('li, article, .cast-item, .member-item, div[class*="cast"], div[class*="member"]');
    const parentText = parentItem.text().replace(/\s+/g, ' ').trim();

    let name = figcaption;
    if (!name) {
      // 名前パターン: 漢字・ひらがな・カタカナの組み合わせ
      const nameMatch = parentText.match(/^([^\d（(【\s]{2,10}(?:[\s　][^\d（(【\s]{2,8})?)/);
      name = nameMatch?.[1]?.trim() || '';
    }
    if (!name) return;
    if (/logo|banner|icon|求人|スタッフ募集/i.test(name + imgSrc)) return;
  ...
  unique.forEach(t => console.log(`  ${t.name} (${t.age}歳, ${t.height}cm, ${t.cup})`));
