/**
 * アロマジラフ セラピスト・ロゴ・スケリンク・料金 一括登録
 * 実行: node insert_giraffe_therapists.mjs
 */
import fs from 'fs';
...
async function run() {
  ...
  const SITE_URL = 'https://aroma-giraffe.com';
  const THERAPIST_PAGE = 'https://aroma-giraffe.com/therapist';
  const SCHEDULE_URL = 'https://aroma-giraffe.com/schedule';
  const WEBSITE_URL = 'https://aroma-giraffe.com';
  const STORAGE_BUCKET = 'shop-logos';
  const LOGO_FILE_NAME = 'AromaGiraffe.png';
