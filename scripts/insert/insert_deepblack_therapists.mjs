/**
 * Deep Black セラピスト・ロゴ・スケリンク・料金 一括登録
 * 実行: node insert_deepblack_therapists.mjs
 */
import fs from 'fs';
...
async function run() {
  ...
  const SITE_URL = 'https://www.deep-black.tokyo';
  const THERAPIST_PAGE = 'https://www.deep-black.tokyo/cast/';
  const SCHEDULE_URL = 'https://fujoho.jp/index.php?p=shop_info&id=65714&h=ON';
  const WEBSITE_URL = 'https://www.deep-black.tokyo';
  const STORAGE_BUCKET = 'shop-logos';
  const LOGO_FILE_NAME = 'DeepBlack.png';

  const price_system = `60分: 1,000円
90分: 2,000円
120分: 3,000円`;

  console.log('🚀 Deep Black セットアップ開始\n');
  ...
  $('.list-staff li').each((i, el) => {
    const imgEl = $(el).find('.photo img');
    const style = imgEl.attr('style') || '';

    // background-image: url(/images/...) から画像URLを取得
    ...
    // alt属性から名前を取得（「えりか (Erika)さんの写真」→「えりか」）
    const alt = imgEl.attr('alt') || '';
    const name = alt.replace(/\s*\(.*?\)/g, '').replace(/さんの写真.*/, '').trim();
