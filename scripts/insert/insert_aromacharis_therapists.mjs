/**
 * AROMA CHARIS (アロマカリス) セラピスト・ロゴ・スケリンク・料金 登録
 * 実行: node scripts/insert/insert_aromacharis_therapists.mjs
 */
import fs from 'fs';
...
async function run() {
  ...
  const SHOP_ID = 'chiba_nishifunabashi_aroma_charis';
  const SITE_URL = 'https://aromacharis.com';
  const MODEL_PAGE = 'https://aromacharis.com/model/';
  const SCHEDULE_URL = 'https://aromacharis.com/schedules/';
  const WEBSITE_URL = 'https://aromacharis.com';
  const LOGO_URL = 'https://aromacharis.com/ogp.jpg';
  const STORAGE_BUCKET = 'shop-logos';
  const LOGO_FILE_NAME = 'AromaCharis.jpg';

  const price_system = `【スタンダードコース】
60分: 14,000円
90分: 17,000円
120分: 22,000円
延長30分: 6,000円
...
延長30分: 6,000円`;
  ...
  console.log('🚀 AROMA CHARIS セットアップ開始\n');

  // Step 1: セラピストページ取得
  console.log('📄 Step 1: セラピストページを取得中...');
  const res = await fetch(MODEL_PAGE, { headers: ua });
  ...
  $('.castimage').each((i, el) => {
    ...
    // .castname は .castimage の次の兄弟要素
    const castname = $(el).next('.castname');

    // 名前: .castname a
    const name = castname.find('a').first().text().trim();
    if (!name) return;

    // 年齢: .kanakana em テキスト "(27)" → "27歳"
    const ageText = castname.find('.kanakana em').text().trim();
    const ageMatch = ageText.match(/\d+/);
    const age = ageMatch ? `${ageMatch[0]}歳` : '';

    // スペック: "T159 90(F)-57-85" 形式
    const specText = castname.find('span:not(.kanakana)').text().trim();
    const heightMatch = specText.match(/T(\d+)/);
    const bustMatch = specText.match(/(\d+)\(/);   // 90(F)
    const whMatch = specText.match(/\)-(\d+)-(\d+)/); // )-57-85
    ...
    if (whMatch) {
      raw_data.waist = whMatch[1];
      raw_data.hip = whMatch[2];
    }
