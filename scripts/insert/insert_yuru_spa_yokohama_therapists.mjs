/**
 * ゆるスパ横浜 セラピスト登録
 * 実行: node scripts/insert/insert_yuru_spa_yokohama_therapists.mjs
 */
import * as cheerio from 'cheerio';
import fs from 'fs';

const SHOP_ID = 'kanagawa_shinyokohama_yuru_spa';
const SITE = 'https://yuru-spa.com/yokohama';
...
async function run() {
  ...
  $('li:has(img[src*="therapist_img"])').each((_, el) => {
    const imgEl = $(el).find('img[src*="therapist_img"]').first();
    if (!imgEl.length) return;

    // alt: "夏実ゆりな ｜横浜メンズエステ 出張型メンズエステ ゆるスパ" → 名前部分のみ
    const rawAlt = imgEl.attr('alt') || '';
    const rawName = rawAlt.split(/[｜|]/)[0].trim();
    ...
    if (imageUrl && !imageUrl.startsWith('http')) {
      imageUrl = imageUrl.startsWith('/') ? `https://yuru-spa.com${imageUrl}` : `https://yuru-spa.com/${imageUrl}`;
    }
    // クエリパラメータ除去
    ...
    const heightM = text.match(/T[．.]\s*(\d{3})|T(\d{3})|(\d{3})\s*cm/i);
    const cupM = text.match(/([A-J])\s*カップ/i) || text.match(/\(([A-J])\)/i);
    if (ageM) raw_data.age = parseInt(ageM[1]);
    if (heightM) raw_data.height = parseInt(heightM[1] || heightM[2] || heightM[3]);
