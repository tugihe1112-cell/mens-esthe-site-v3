...
> **最終更新: 2026-09-08 （指示書の作業単位1・2を実装しpush。F01は静的最適化ページの問題まで含めて解決）**
> 作業がひと段落するたびに、Codexがこのファイルを自動更新する。

---

## 2026-09-08 指示書の作業単位1・2を実装（`b65fb1c` / `ef2450f` / `e8d8389`）
...
- **F01の追加修正** = `/login` `/register` `/admin` `/auth/*` は `○ Static` で `router.asPath` にクエリが載らず、**実装済みのF01が本番で効いていなかった**。本番実測で確認し、compatの `resolveQueryString` で実URLから補う形に修正（`e8d8389`）。`/admin?review=`（新着口コミメール導線）も同時に直った。
- **本番設定** = Supabase の Redirect URLs に `/auth/complete` を2件（完全一致＋クエリ付き照合用）追加済み。Site URL は本番URLで正しい。
- **未実施** = ブラウザでの見た目確認（360/390/768/1024/1440px）と、デプロイ後のF01の実測。作業単位3以降（U01・U03 →U02・U04 →U05 →F04・F05 →F08・F09 →U06）は未着手。
