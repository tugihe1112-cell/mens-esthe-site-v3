# mens-esthe-site（サイト名: メンエスマップ） — Codex引き継ぎドキュメント
...
## 2026-09-09 指示書の作業単位4〜8を実装（残り全部）

- **単位4（U02・U04）** = ホームの入口と、口コミ読了後の登録導線。可視H1を検索カードへ（`sr-only` H1は削除）、地域別口コミは select＋最大2件、重複バナー群を削除して投稿バナーを下部1か所へ。読了後の案内は `RegisterInvite` に集約し**未登録／閲覧権あり／期限切れ**で出し分ける。ホーム口コミカードの「続きを読む→300字取得→全文を読む」の二段階を、**全文リンク1回**（`#review-<id>` 付き）に置き換えた。
- **単位5＋6（U05・F04・F05）＝1コミットにまとめた。** 理由：U05の「口コミ件数はF04で人物一致が保証されたものだけ」という条件で、`SearchPage` の同じ集計コードを両方が書き換えるため。
  - **F04** = 名前による人物照合を全廃。契約は **`src/utils/reviewIdentity.js`** の1か所だけ。基本キーは `therapist_id`／IDがある口コミは完全一致だけ／IDが無い旧口コミは同一店舗に同名が1人のときだけ／異なるIDを同一人物とみなす対応表は作らない。**人物SSRの「IDが一致 または 名前が一致」というORが最悪**で、弾いた別人を名前で拾い直していた。
  - **F05** = 通信失敗を空配列に丸めない。`PopularReviewsPage` の `loadMore` が**取得前に offset を+20**していたため、失敗すると次の20件が永久に飛んでいた。
- **単位7（F08・F09）** = 登録特典の付与を**読み戻して確認**してから確認メールを送る。F09は読み取り調査のみで、結果は `docs/uiux-audit-2026-09-06/F09-data-findings.md`。**本番DBは1行も変更していない。**
- **単位8（U06）** = 登録ファネルの8イベント。`source` とエラー分類は固定の許可値のみ、`registration_return` は `page_type` に畳む、`/auth/*` の自動page_viewから token/next を落とす。
- **未実施** = 指定幅（360/390/768/1024/1440px）の目視確認。**人の目が要る唯一の残作業。**

### ガードを書くときの再確認（4回踏んだ）

**参照ではなく定義を見る。** `var(--ui-primary)` / `SAFE_API_MESSAGE_STATUS.has()` / `if (!grantedOk)` /
「filterReviewsForTherapist を1箇所でも呼んでいるか」——いずれも**定義や実際の使用箇所を消しても参照だけが残って素通り**した。
必要なら**件数を数える**。そして**新しいガードは必ず妨害テスト（わざと壊して落ちるのを確認し、戻す）で検証する。**

---

## 🚨 2026-09-08 本番事故：新規登録が約21時間できなかった（`32bae95` で復旧）

- **原因** = `api/auth/signup.js` が `src/utils/authRedirect.mjs` を import していた。**Vercelはリポジトリ直下の `api/` を CommonJS にコンパイルする**ため、その import は `require()` になる。**`.mjs` は常にESMなので `require()` できない。** 本番ログ＝`Error [ERR_REQUIRE_ESM]: require() of ES Module /var/task/src/utils/authRedirect.mjs`。関数が起動せず `/api/auth/signup` が Next の静的500 HTML を返していた。
- **修正** = `.mjs` → `.js` にリネーム（12ファイル15箇所）。**`api/` から共有したい処理は必ず `.js` に置くこと**（`server/rateLimit.js` と同じ）。Node 24 は `.js` 内のESM構文を自動判別するのでCIから直接importできる。`check_core_safety_guards.mjs` に `api/`・`server/` 配下の `.mjs` import 検出を追加した。
- **🚩ここが一番重要** = **`npm run build` は `api/` を検査しない。** ビルド成功・全14ガード通過・lint通過・構文チェック通過が**すべて緑のまま本番だけが落ちていた**。ローカルの `node` は拡張子を解決できるので手元でも再現しない。
- **教訓** = 書き込み系の経路（登録・投稿・メール送信）を触ったら、**デプロイ後に実際に送信して応答を確かめるまで完了と呼ばない。** 前日の報告は画面の見た目とGET応答だけを見た「全部OK」で、**フォームを一度も送っていなかった**。
- **復旧確認** = GET `/api/auth/signup` → 405、POST（空ボディ）→ 400 `application/json`。実メールでの登録も「確認メールを送信しました」まで到達。

---

## 2026-09-08 指示書の作業単位1〜3を実装（`b65fb1c` / `ef2450f` / `e8d8389` / `74081de` / `ebd7125` / `11d5ca5`）

- **作業単位1（F01・F02・F03）** = 登録後に元の口コミへ戻れない／確認メールの二重検証／投稿の文字数メーターと判定の食い違い。`b65fb1c`。
- **作業単位2（F06・F07）** = 根拠のない「口コミ人気 No.」「★ New」「近く」表示とヘッダー透過判定／出勤表のCSPエラー。`ef2450f`。
- **作業単位3（U01・U03）** = 共通トークン `.ui-*`（`src/index.css`）とナビの整理／登録・ログインフォームの作り直し。`ebd7125`。**768〜1023pxで主ナビがどこにも無かった**（ヘッダーPCナビは `lg:` から、BottomNavは `md:hidden` だった）のを `lg` 境界へ統一。
- **⚠️F01の診断を1度間違えている（記録として残す）** = 当初「`/login` `/register` は `○ Static` なので `router.asPath` にクエリが載らない」と結論して `e8d8389` を入れたが、**これは誤り**。本番実測では `router.asPath` にクエリは載っており `router.query` も正しかった。根拠にした `__NEXT_DATA__.query` が `{}` なのは**ビルド時にHTMLへ焼かれた値**であって `router.query` ではない。**本当の原因は、Reactがハイドレーション時の属性の食い違いを直さないこと**（本番ビルドでは警告も出ない）。静的HTMLに焼かれた `href="/register"` が、正しい値を計算しても書き換わらなかった。修正は `useRequestedReturnTo`（マウント後に読む二相フック）へ寄せた＝`74081de`。**`e8d8389` の fragment 除去は独立した強化なので残している。**
- **本番設定** = Supabase の Redirect URLs に `/auth/complete` を2件（完全一致＋クエリ付き照合用）追加済み。Site URL は本番URLで正しい。
- **未実施** = ブラウザでの見た目確認（360/390/768/1024/1440px）。作業単位4以降（U02・U04 →U05 →F04・F05 →F08・F09 →U06）は未着手。
- ⚠️ 同じリポジトリを複数セッションが同時に触ることがある。**コミット時は `git add -A` を使わず、触ったファイルだけを明示指定すること。**

---

## 2026-09-08 失敗の理由を利用者に説明する（`11d5ca5` ＋ 追加修正）

- **原則** = 「内部の例外文を出さない」と「理由を説明しない」は**別の話**。片方に全部倒すと必ずもう片方が壊れる。**状態コードで線を引く。**
  - **4xx と 503** … APIが利用者向けに書いた日本語文なので、画面はそのまま表示する（`SAFE_API_MESSAGE_STATUS`）。
  - **500 と通信失敗** … `err.message` が混ざるので**絶対に出さない**。固定文言へ写す。
- **登録画面**（`11d5ca5`）= 一度**4xxまで全部を1つの固定文言へ潰して**しまい、`+t1@gmail.com` で登録できない理由が利用者にも我々にも分からなくなった。上の線引きへ修正済み。
- **ログイン画面 ＋ 登録API**（追加修正）= 分類と文言を **`src/utils/authErrorText.js`** に集約（`.jsx` ではなく `.js` ＝ CIから実際に呼んで検査するため）。5分類それぞれに `text`（何が起きたか）と `hint`（次の一手）を持たせ、**分類できない例外は `server` へ倒す**。項目別エラー（`aria-invalid`／`aria-describedby`／最初の不正欄へフォーカス）も追加。登録APIは `email_address_invalid`・`weak_password` を **400＋日本語**で返し、**500の応答本文から `err.message` を削除**した。
- **ガードを書くときの注意** = **参照ではなく定義を見る**こと。`var(--ui-primary)` の参照が残っていてトークン定義の削除を見逃す、`SAFE_API_MESSAGE_STATUS.has()` の呼び出しだけ残して定義の削除を見逃す、という取り違えを09-08だけで3回やっている。**新しいガードは必ず妨害テスト（わざと壊して落ちることを確認し、戻す）で検証する。**

---

## 2026-09-07 UI/UXコンサル指示書（作成時点では実装未着手。実装は09-08に完了）

- オーナー依頼：「実装しなくていい」「他のモデルでもできるように、どこをどう変えるか・コードを細かく指示」。消費を抑えるため、実装作業単位に分けて文書化した。
- 成果物：`docs/uiux-audit-2026-09-06/START.md`（依頼用プロンプト・順序・検証範囲）、`DESIGN.md`（スマホ/PCの具体仕様）、`FIXES.md`（不具合の根拠・対象コード・修正契約）。現行画面11枚も同フォルダに保存。
- 本番画面の登録訴求・固定順位表示・出勤iframeエラー等と、コード上の登録復帰欠落・確認effect再実行・投稿文字数不一致・同名人物照合等を区別して記載。本番登録/メール送信、DB全件監査は未実施。
- サイトコード・DB・本番設定・デプロイは変更していない。提案内容をオーナー確定事項へ追加していない。開始時からの`playbook/metrics-log.md`変更はそのまま保持。

---

## 2026-08-30 GitHub Actions監視の連続誤報を根本修復

- 調査:
  - `Monitor (chunk integrity)`が#616〜#622の7回連続で失敗通知を送信
  - 公開Actions APIで全7回を照合すると、JSチャンク・HTTP境界・API契約は毎回成功し、失敗は8/29追加の「ページ・内部リンク・画像」検査だけ
  - 本番はNode 22で主要+sitemap 64ページ、内部リンク120件、13固有JSチャンクを再実行して正常。サイト停止や真っ黒事故ではなかった
- 原因:
  - ホームの固定ヒーロー3画像だけが`linda-spa.com` / `aromacharm.net` / `tokyoaroma.jp`へ直リンクしており、GitHub Runnerから第三者ホストへの取得差で監視が落ちる構成だった
  - 監視はネットワーク例外しか2回再試行せず、408/425/429/5xxを即時失敗。64ページ＋画像＋内部リンクを同時16本で重複取得していた
  - ワークフロー名がchunk障害に見える一方、実際はページ画像検査の失敗。`actions/*@v4`のNode 20廃止警告も併記され、通知内容がさらに紛らわしかった
- 修正:
  - 3画像を同一バイトのまま自社Cloudflare R2へ保存し、ヒーローと旧JSONの直リンクをR2へ統一。旧AromaCharmフォールバック写真は既に404だったため正常なR2ロゴへ置換
  - `monitorFetch.mjs`へDNS/接続例外と408/425/429/5xxの3回再試行を集約。恒久4xx・非画像MIME・壊れたJSは従来どおり必ず失敗
  - ページ巡回を同時8本、画像6本へ抑え、既に検査したrouteの内部リンク再取得を除外
  - JSチャンクは5ページで重複していた49取得を13固有URLへ集約。404だけデプロイ境界を考慮して3回確認し、恒久404は失敗を維持
  - HTMLは一般利用者と同じキャッシュ経路で取得。`no-cache`により利用者だけが踏むstale HTMLを監視が回避する矛盾を解消
  - ワークフローを`Monitor (production integrity)`へ改名し、CI/監視/画像監視を公式最新版`checkout@v7`・`setup-node@v7`へ更新
- 再発防止:
  - prebuildへ5xx→成功、DNS例外→成功、通常404即時失敗、chunk 404再確認、恒久5xx失敗の単体検査を追加
  - 固定ヒーローと旧JSONへ対象公式サイト画像の直リンクが戻る変更、再試行削除、重複chunk取得、`no-cache`復活、旧Actions復帰をCIで拒否
- 検証: Node 22で監視4本、監視耐障害テスト、lint、30ページbuild、npm audit 0、ローカル主要28ページ／内部リンク96件が成功。R2の3画像は元画像とSHA-256が一致
...
- 依存・継続監視:
  ...
  - `.vercelignore`を追加し、ローカル`.env`・派生バックアップ・GCP鍵・生成物をCLIデプロイのアップロード対象から除外。欠落時はprebuildを失敗させる
...
- 一般会員:
  - お気に入りをユーザー別へ分離し、IDに`_`を含む店舗・セラピストでも正しく復元。店舗だけ保存した場合は店舗タブから開く
  - マイページと投稿履歴を実DBへ接続し、非公開を含む本人の口コミ・実plan・creditsを表示。デモ会員・ローカル偽口コミを撤去
  - パスワード再設定画面と回復セッション処理を追加。認証確認後の遷移先は同一サイト内だけを許可
- 管理人:
  - 新着口コミメールを`/admin?review={id}`へ変更し、未ログインならreview IDを保持してログイン後に対象カードを展開・スクロール
...
- DB:
  - migration `harden_community_rls_and_integrity` と `consolidate_read_policies_and_indexes` を本番適用
  - 掲示板・チャットの本人名義、room組合せ、送信者、参加者閲覧をRLSとCHECK/FKで強制。重複read policyとadvisor指摘を整理
  - 2ユーザーのREST/RLS E2Eで、自分の非公開口コミ、他人の非公開拒否、投稿削除権限、チャット参加者閲覧、送信者偽装拒否を確認
- 画像の再々監査:
  - HTTP 200でも中身がHTMLの旧R2オブジェクトを全URLの実バイト検査で発見。店舗10行＋セラピスト466行（38店舗系統）を対象一致付きでnullへ正規化
  - 修復後の全件走査は店舗756 URL／セラピスト32,665 URLで配信異常0。既知誤画像・広告名・同一画像の別人利用も0
  - 日次サンプルを40→100へ増加し、週1回は全DB画像をMIME＋マジックバイトで検査。結果逐次保存・通信再試行・dry-run修復スクリプトを追加
- 依存・品質:
  - Next.js 16.3.2、React 19.2.8、`@vercel/og` 1.0.1へ更新し、`npm audit` 0件
  - Next正式の`next/head`へ統一し、旧Vite入口・デモログインバックアップ・`legacy-peer-deps`を削除
  - ESLint flat configへ移行し、全hook依存を修正。lint警告0、30ページbuild成功
- 三重検証:
  - 390pxの主要27ルート: 横はみ出し0・表示中の破損画像0・実行時エラー0・title欠落0
  - 1440pxの主要21ルート: HTTP 200・横はみ出し0・破損画像0・title正常
  - 初回客（匿名）／一般会員A／別会員B／管理者境界を独立確認。お気に入り分離、非公開口コミ、チャット、管理画面拒否が想定どおり
  - 回帰ガードをprebuildへ追加し、既知事故・権限・深いリンク・全画像監視の欠落をデプロイ前に停止
...
## 2026-08-22 口コミの区分フォーマット再発防止

- 実ユーザー初投稿で、投稿画面は「入店・受付／ご対面／施術・接客／総評」の4欄だったのに、管理画面・公開画面では本文が一続きになっていた
- 原因: `useReviewForm`が`Object.values(data.story)`の値だけを連結し、どの欄の本文かを保存していなかった。2026-03-28の初期実装から存在
- 対応:
  - `reviews.story_sections jsonb`を追加し、4区分を本文とは別に保存。既存口コミは変更せず`NULL`のまま
  - DB制約で許可キー・文字列型・`story_sections`から再構成した本文と`content`の完全一致を検査し、直接RESTから表示内容と文字数判定を食い違わせる操作を拒否
  - `ReviewStoryContent`を公開口コミカードと管理画面で共用。新規投稿は区分見出し付き、旧`【見出し】`口コミも従来表示を維持
  - 入力メーター・Zod・DBの最低200字・credits付与を、各欄をtrimした本文文字数の合計へ統一。自動挿入する区分間の空行は200/700字へ加算しない
- 本番DB:
  - migration `review_story_sections_sha_9c6d152` / version `20260821232053`
  - migration `review_story_character_count_sha_72d5480` / version `20260821232245`
  - 200字の構造化投稿は成功、199字と本文不一致はCHECK拒否をトランザクションE2Eで確認し全ロールバック。テスト残骸0件
  - 初投稿は`story_sections=NULL`・910字のままで、既存本文を変更していない
- 再発防止: `scripts/ci/check_review_story_format.mjs`をCIへ追加し、保存・表示・DB整合性制約・文字数判定が欠けたら失敗。`playbook/decisions.md` D-009にも固定
- 検証: 全CIガード・`git diff --check`・`npm run build`成功。本番投稿画面で4欄へ各1字を入力し、生成する空行を含めず合計4字・残り196/696と表示することを実測後、テスト下書きを破棄
- 本番: commits `9c6d152` / `72d5480`、Vercel Production `dpl_3FjZnj7rxoKZL6MgtW2Du6AQtbvU` Ready

---

## 2026-08-22 新着口コミ通知→管理画面の導線修復

- 実ユーザーの初投稿（`r_1787333476619_i1oi0`、900字以上）で、管理者通知メールのボタンから管理画面を開けないことが判明
- 原因1: メールアプリ内ブラウザは管理者のSupabaseセッションを持たないが、`AdminPage`は認証確定後の`user=null`でもログインへ送らず`null`を返していたため空画面になった
- 原因2: `notify-review`へ保存済みreview IDを渡しておらず、メールは単なる`/admin`リンクだったため、開けても対象口コミを探す必要があった
- 修正:
  - 旧メールの`/admin`でも、未ログインなら`/login?redirect=%2Fadmin`へ送り、ログイン後に管理画面へ戻す
  - 新規メールは`/admin?review={reviewId}`へ直リンクし、認証後に対象カードを自動展開・強調・中央へスクロール
  - リンク対象が新着200件より古くてもIDで追加取得し、過去メールの深いリンクを維持
  - ログイン後redirectは`/`1文字で始まる同一サイト内パスだけを許可し、外部URLへのオープンリダイレクトを防止
...
## 2026-08-21 全サイトのスマホUI/UX改善

- 390×844pxで公開・認証・投稿系の主要17ルートを実表示監査。全ルートで横はみ出しなしを確認
- `_document`とNext.js既定のviewportが二重になっていたため、`_app`の1本へ統一。`viewport-fit=cover`は維持
- 共通ヘッダーをスマホだけ小型化し、BottomNavと重複していたログインを外して「無料登録」へ集約
- BottomNavを高さ56px・下余白8pxへ小型化。チャットルームでは専用送信欄を隠さないよう、フッターと共に非表示
- ホームはスマホだけ「店舗・エリア／セラピスト」切替＋入力1本に統合。PCの2入力AND検索は維持
- ヒーローをスマホだけ低くし、検索と最新口コミが初期画面に入るよう圧縮。インライン`style`をCSSへ移しhydration不一致も解消
- 店舗一覧はスマホだけ横長カード（約134px）へ変更。PCの3列カードは維持
- 店舗詳細はヒーロー、見出し、固定ナビを圧縮し、口コミCTAとセラピスト2枚を初期画面内へ配置
- 口コミタグが0件の店舗では絞り込みを非表示。タグあり店舗は背景スクロールを止めるボトムシートで選択
...
## 2026-08-15 `/search`スマホUI/UX改善

- 改善前の問題:
  - 店舗・キャストの2入力欄、結果内のキャスト名入力欄、折り返すソートが縦幅を使い、390×844pxでは最初の写真がほぼ画面外
  - 未入力時にも「絞り込み」が出る一方、タグ欄自体は非表示のため押しても意味がない
  - モバイルのタグ欄は結果より前へ長く展開され、結果へ戻りにくい
- 対応:
  - スマホだけ「店舗・エリア / セラピスト」の対象切替＋入力欄1本に集約。店舗を残したままセラピストへ切り替えると複合検索できる
  - 結果内の重複する名前入力欄はスマホで非表示、ソートは短縮ラベルの横スクロール1行へ変更
  - タグが1件以上ある場合だけ「条件で絞り込む」を表示し、背景を遮るボトムシート内で選択して「完了」で戻る構成へ変更
  - 重複していた未入力時ステータス、余白、説明カード、カード内文字を圧縮し、2枚の写真を初期画面内へ配置
  - 口コミ投稿帯をスマホでは縦積み＋全幅ボタンにし、付与条件を「200字で3日・700字で7日」へ明記。挿入位置も2/3/4/5列の行を崩さない位置へ変更
- 保持したもの: デスクトップの店舗＋キャスト2欄検索、Supabase取得条件、タグのAND条件、24件ずつの「もっと見る」
- 検証: 390×844pxで未入力24件・写真2枚が初期画面内、MoMo Spa＋佐藤の複合検索1件、天海ゆら検索でタグ絞り込みボトムシート開閉・選択後1件を確認。1440×900pxでも従来の2入力欄と24件表示を確認。デザイン決定CI・`git diff --check`・`npm run build`成功

---

## 2026-08-15 `/search`未入力時の注目セラピスト一覧

- 原因: ホームの「注目セラピスト > もっと見る」とヘッダーの「キャスト検索」は`/search`へ遷移するが、検索語が空の場合は取得結果を意図的に0件にしていたため、遷移先が入力案内だけの空ページになっていた
- 対応: 検索語が空でも、写真あり・在籍中のセラピストから注目一覧を表示するよう`SearchPage`を変更
  - 店舗ごと最大2名、都道府県・地域を巡回して特定店舗への偏りを抑制
  - 初期24件、既存の「もっと見る」で24件ずつ追加表示
  - 未入力時は件数0のタグサイドバーを隠して5列グリッドを使用
  - 店舗名・セラピスト名を入力すると、従来の店舗照合・タグ・並び替え付き検索へ切り替わる
- 再発防止: `playbook/decisions.md`へD-007を追加し、`check_design_decisions.mjs`で未入力一覧の実装と案内文の削除を検知
- 検証: デザイン決定CI・`git diff --check`成功、`npm run build`はコンパイル・30ページ生成成功。実ブラウザで未入力24件→「もっと見る」48件、MoMo Spa検索への切替、未入力時のサイドバー非表示を確認

---

## 2026-08-14 全店の画像出所・別人画像衝突を再監査

- 結論: 「まとめサイト由来」はMoMoの主因だったが、公式サイト由来でも誤画像は起きていた
  - MoMo: 公式`momospa.tokyo`ではなく、同名の別サイト`momospa.net`の名簿を誤結合
  - 複数店舗: `<img src>`の透明spacer・準備中画像を取得し、実写真の`background-image`を見落とし
  - LOHAS金沢/沖縄: 両方とも公式サイトだが、R2保存名が`lohas_{番号}`で店舗を含まず相互上書き
  - DAHLIA: 動画を`.jpg`として保存した旧データが1件
- 本番修復:
  - 全店を「同一店舗で無関係な名前が同じ画像」「無関係な店舗間で同じ画像」「既知placeholder/動画」で再監査し、誤った画像割当174件をnull化、空文字URL7件もnullへ正規化
  - LOHAS金沢31名・沖縄25名を各公式名簿から再取得し、`shop_id`を含むR2キーで本人写真56枚を再保存
  - 広告/UIテスト等の偽レコード4件は口コミ参照0件を確認して削除。LOHAS金沢の旧在籍行は公式名簿に同期して非在籍化
- 再発防止:
  - 名簿ページは店舗公式ドメインまたはそのサブドメインだけを許可（画像ファイル自体は公式が使うCDN/S3を許可）
  - `scripts/lib/sourceProvenance.mjs`で公式名簿の出所検査と店舗単位のR2キー生成を共通化
  - 日次監視を「5名以上の大量重複」から「無関係な名前2名以上の同一画像」まで厳格化し、既知placeholderのURL/内容ハッシュ/ファイル名も拒否
- 再実行用:
  - 読み取り監査: `scripts/debug/audit_therapist_source_provenance.mjs`
  - 修復: `scripts/maintenance/repair_semantic_image_collisions.mjs`（既定dry-run、`--live`のみ更新）
  - 公開検証: `scripts/debug/verify_lohas_image_repair.mjs`
- 最終検証: active 60,727件で既知誤画像0、同一店舗の別人画像衝突0、無関係な店舗間衝突0、広告名0。LOHASは匿名RLSで金沢31名/沖縄25名を取得し、56画像すべてHTTP image配信成功。日次ランダム画像80件も配信NG 0
- 修復後総数: shops 1,099／therapists 60,882／reviews 16

---

## 2026-08-14 MoMo Spa誤名簿・全店誤画像修復

- 対象: `60350`（MoMo Spa）。公式は `https://www.momospa.tokyo/girl`
- 原因: 店舗公式は`momospa.tokyo`なのに、旧セラピスト97件は別サイト`momospa.net`由来。存在しない店舗IDにも同じ97件が重複し、広告文・準備中・写真NG画像を人物として取り込んでいた
- 全店監査: 同一画像の大量使用を抽出して実画像を目視。広告・ロゴ・空画像・風景・別人使い回しの18画像、32店舗ID・828行を誤画像と確定
- 本番修復:
  - MoMoを公式現役34名へ置換、本人写真32枚をR2へ保存、公式も準備中の2名は`image_url=null`
  - 口コミ参照0件を確認して旧MoMo名簿194件を削除
  - 全店の確定誤画像828件は`image_url/profile_image=null`へ変更。MoMo以外の名前レコードは口コミ投稿用に保持
  - 別店舗に残っていた広告名レコード「即姫割」1件も、口コミ参照0件を確認して削除
- 再実行用: `scripts/maintenance/repair_momospa_and_placeholder_images.mjs`（既定dry-run、`--live`のみ更新）
- 再発防止:
  - `scripts/lib/therapistImageQuality.mjs`: 既知誤画像URL、広告/準備中のファイル名、目視確認済みプレースホルダーのSHA-256を共通判定
  - `scripts/lib/r2Upload.mjs`: therapist画像のアップロード前後でURLと画像内容を検査し、広告・ロゴ・準備中画像をR2へ入れない
  - 日次`check_image_health.mjs`: 全在籍行の既知誤画像・広告名、1店舗5名以上への同一画像使い回しが1件でもあれば失敗
  - CI: `check_therapist_image_quality.mjs`で判定規則の回帰を防止
- 最終検証: MoMo現役34名／写真32名／公式も写真なし2名、匿名RLSから同数取得、32画像すべて配信正常。全表示画像51,019件で既知誤画像0・広告名0・5名以上使い回し0
- 修復後総数: shops 1,099／therapists 60,886／reviews 16
...
## 2026-08-14 UI/UX画面設計ガイドの選別実装

- 外部の「UI/UXワイヤーフレーム・画面設計ガイド」を既存実装と `playbook/decisions.md` に突合し、P0/P1のうち矛盾しない部分を実装
- 口コミカード（`ModernReviewCard`）
  - 保存されている場合だけ来店月・コース・総額を最上段の証拠行に表示し、投稿日を来店日として代用しない
  - 「実体験レポート」・数値評価・投稿者を簡潔に表示し、6軸評価をモバイルでも2行×3列へ統一
  - ロック時の強いCTAを「体験談を投稿して続きを読む」の1本に絞り、タップ領域とフォーカス表示を改善
- セラピスト詳細
  - モバイル写真を最大220pxに抑え、重複していた店舗リンクを削除して最初の口コミを初期画面へ近づけた
  - 6軸平均を2行×3列へ統一し、投稿特典の誤表記「即7日」を実仕様の「200字で3日・700字で7日」に修正
- ホーム
  - 検索直後を「最新の実体験口コミ1件 → 中立性・掲載母数 → 地域別口コミ」の順に変更
  - 投稿訴求は地域別口コミを読んだ後に維持し、先頭の強い投稿バナーは削除
- 投稿画面
  - 店舗・セラピスト検索で全角/半角・空白・カタカナ/ひらがなの揺れを吸収
  - 同名系列店を判別できるよう候補に都道府県・エリアを表示
  - Step 1で「200字3日・700字7日」と送信前確認を明示
- **見送り**
  - 店舗ページのタブ/セグメント化はD-001（左タグサイドバー＋キャスト一覧を維持）と衝突するため不採用
...
- [x] **DNS反映後の作業（mens-esthe-map.jp）**（2026-05-30）
  - `https://www.mens-esthe-map.jp` が 200 で開けることを確認
  - `https://mens-esthe-map.jp` は `https://www.mens-esthe-map.jp/` へ 307 リダイレクト
  - Vercel 環境変数 `VITE_PUBLIC_SITE_URL` は `https://www.mens-esthe-map.jp` に更新・Redeploy済み
  - `public/sitemap.xml` / `public/robots.txt` のドメインを `https://www.mens-esthe-map.jp` に更新済み
...
- [x] **お問い合わせフォームの実装**（2026-05-30）
  - `/contact` を追加
  - `api/contact.js` で Resend 経由のお問い合わせ通知を送信
  - `/legal` の「お問い合わせフォーム」リンクと Footer の導線を追加
...
| **神奈川県** | Fromage: JS描画のためCodex in Chrome必要 | — |
...
7. **AGENTS.md更新**: 完了したら都道府県別テーブルと作業ログを更新
...
- **愛知県 完了**
  ...
  - ⚠️ JS描画4店舗（Aroma Terrace, Cucue, century, ゆりかご）は未処理。URL収集済み、Codex in Chrome が必要
...
- **福岡県 完全完了**
  - メンズアロマ博多人妻さん (126名) 挿入 → `process_hakata_hitozuma.mjs` + `fix_hakata_hitozuma_images.mjs`
    - Codex in Chrome で hakatahitozuma.com/staff/ をJS描画取得
...
- **宮城県 7/8店舗完了**
  ...
  - Aroma No5: Wix JS描画のため未処理（Codex in Chrome 必要）
...
- **東京都 グループ4社 完了**
  ...
  - S3 URLは定期的にUUIDが変わる仕様 → 403が出たらCodex in ChromeでDOM抽出して `fix_tokyo_groups_retry.mjs` で再試行
...
- **ヘッダー/ナビ ラベル統一（Codex + Codex協働）**
  - `/search` がキャスト検索ページである実態に合わせて文言統一
  - PCヘッダー「店舗検索」→「キャスト検索」（Codex）
  - BottomNav「探す」→「キャスト」（Codex）
...
- `TRANSFORMED` 配列に `{ therapist_name_match, content, detailed_ratings, rating, tags }` を定義してCodex自身が書き直す
...
- **目的**: スクレイピング後に自動でCodex API（Haiku）で v2品質書き直しを実行
- **事前準備**: `npm install @anthropic-ai/sdk`（初回のみ）
- **フロー**: `user_id='menesthe_import'` を取得 → Codex Haiku API で書き直し → DB更新 + `user_id='menesthe_rewritten'` に変更（再処理防止）
- **オプション**:
...
- 142件の失敗分をCodex in Chromeで各サイトを開いてog:imageを手動収集
...
- **JS描画サイト**はcheerioでは取得不可。Codex in Chromeブラウザ経由が必要
- サンドボックス（Codex側のbash）は外部ネットワーク不可。スクリプトはユーザーのターミナルで実行
...
- 更新ファイル: `index.html` / `src/components/SeoHead.jsx` / `public/manifest.json` / `AGENTS.md`
...
### 2026-05-30 - 公開後チェック・本番URL統一・お問い合わせフォーム追加

#### 公開URL確認
- `https://www.mens-esthe-map.jp` が HTTP 200 で開けることを確認
- `https://mens-esthe-map.jp` は `https://www.mens-esthe-map.jp/` へ 307 リダイレクトされることを確認

#### sitemap / robots / SEO URL修正
- `public/sitemap.xml` の全URLを `https://www.mens-esthe-map.jp` に更新
- `public/robots.txt` の Sitemap URL を `https://www.mens-esthe-map.jp/sitemap.xml` に更新
- `src/components/SeoHead.jsx` の `VITE_PUBLIC_SITE_URL` 未設定時フォールバックを本番URLに更新
- `api/notify-credit.js` のメール内リンクのフォールバックを本番URLに更新

#### お問い合わせフォーム追加
- `src/pages/ContactPage.jsx` を追加し、`/contact` ルートを作成
- `api/contact.js` を追加し、Resend 経由で問い合わせ内容を通知
- `src/pages/LegalPage.jsx` のお問い合わせ表記を `/contact` へのリンクに変更
