...
async function run() {
  try {
    ...
    console.log("🛠  最終的なデータ修復とファイル生成を開始します...");
    ...
    yuruRaw.trim().split('\n').forEach(line => {
      ...
      const tid = `yuru_spa_${name}`;
      yuruShop.therapists.push(tid);
      allTherapists.push({
        id: tid, name, age, T, shop_id: yuruShop.id,
        image: img ? `https://yuru-spa.com/gotanda/therapist_img/${img}` : "",
        tags: tags ? tags.split('/') : [], sns
      });
    });

    // 4. メンエスグループ店舗データの作成/読み込み
    ensureDir(menesShopPath);
    let menesShop = {
    ...
    menesRaw.trim().split('\n').forEach(line => {
      ...
      const tid = `menes_group_${name}`;
      menesShop.therapists.push(tid);
      allTherapists.push({
        id: tid, name, age, T, B, shop_id: menesShop.id,
    ...
    console.log("👉 [ npm run sync ] を実行して反映させてください。");
