...
| 状態 | 作業内容 | メモ |
|------|----------|------|
| ⏳ | **🚨D-001違反の再発＝「タグ0件の店舗でキャスト一覧が220pxに潰れる」（要build→push）** | okabayashi「治ってない／以前作った右にタグ・真ん中に写真の形式に戻して／このミス何回も起きてる」。**⚠️まず自分の見立てが2つ外れた**＝①「タブUIが復活した」と考えたが`activeTab`は存在せず、画面上部の「キャスト/口コミ/店舗情報/出勤」は2026-08-08に**アンカースクロールへ変更済み**のもので正常②「CIガードの不備」と考えたが`TAG_CATEGORIES`も残っておりガードは正しく通っていた。**真因**: `a7f7681`（2026-08-21 スマホUI改善）で「タグが0件なら絞り込みを出さない」という`hasAvailableTags`の条件分岐を入れた際、**グリッドの列定義を直し忘れた**。`<div className="grid grid-cols-1 lg:grid-cols-[220px_1fr] gap-6">`が**常に2カラムを宣言**しているため、サイドバーが描画されない店舗では**キャスト一覧が220pxの1列目に落ち**、写真が細く潰れて右半分が空白になっていた。LINDA SPA(60203)は口コミ0件＝タグ0件なので該当。**口コミのある店舗では正常に見えるため気づきにくい**。**修正**: 列定義をサイドバーの有無に連動させる（`hasAvailableTags ? 'grid-cols-1 lg:grid-cols-[220px_1fr]' : 'grid-cols-1'`）。**CIガード追加**: `hasAvailableTags`を使っているのに`lg:grid-cols-[220px_1fr]`が固定文字列で書かれていたらビルドを落とす。列定義を固定に戻して検知されることをテスト済み。**教訓**: **要素を条件付きで消すときは、その要素を前提にしたレイアウト指定（グリッド列・flex比率・余白）も同時に条件付きにする。**ここを分けて書くと、片方だけ直した状態が「特定の条件でだけ壊れる」形で残り、開発時の店舗では再現しない。 |
