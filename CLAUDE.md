...
| ⏳ | **棚卸し分のコミット・push** | 対象＝`playbook/decisions.md`(D-005撤回＋全項目の根拠表)・`playbook/ng-rules.md`(記号導入＋文体制約の削除)・`playbook/persona.md`・`playbook/glossary.md`・`playbook/references.md`＋未追跡の`REVIEW_FORMAT_SPEC.md`。**ドキュメントのみでコード変更なし**。CIガード（`check_design_decisions.mjs`）はパス確認済み。 |
