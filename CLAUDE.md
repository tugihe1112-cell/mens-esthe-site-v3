...
| ✅ | **Tier 4-1：静的ヒーロー化（モバイルLCP/CLS）** | `TopHeroSlider.jsx`のcoverflow Swiperを撤去し**軽量フェード式**に刷新。第1スライドがSSRで即描画（LCP安定）・絶対配置opacityフェードで位置が動かない（**CLS 0.165→ほぼ0＝唯一オレンジのCWV緑化**）・Swiper依存除去でバンドル減・アクティブのみpointer-events:autoでクリック不具合が構造的にゼロ。自動送り4.5s＋ホバー停止＋ドット手動切替。データ取得(heroItems/initialHero)は維持。見た目は「両隣チラ見せ」→「中央1枚クロスフェード」に変化。⚠️デプロイ後に実機検証（モバイル幅含む）してダメなら戻す方針。 |
