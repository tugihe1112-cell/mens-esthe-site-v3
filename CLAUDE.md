...
| ⏳ | **未コミット分のpush（要`git pull`先行）** | `git status -sb`＝`main...origin/main [behind 2]`（bot commitの画像健全性履歴が2本）。**必ず`git pull --rebase`してからpush**（08-31に手順に入れ忘れて`non-fast-forward`で弾かれた実績あり）。対象＝`scripts/metrics/index_coverage_probe.mjs`（セラピストページ検査とページ種別ごとの登録率集計＝**本日の実測を再現できる唯一のコード**）・`CLAUDE.md`・`playbook/metrics-log.md`・`playbook/decisions.md`（D-012）・`playbook/lessons.md`＋未追跡の`CONSULT_REQUEST_SEO_2026-09.md`・`AREA_DEMAND_RANKING_2026-09.md`。**コード変更は測定スクリプトのみでアプリ本体に影響しないが、`npm run build`は通してからpushすること**（CIガード13本が走る）。 |
