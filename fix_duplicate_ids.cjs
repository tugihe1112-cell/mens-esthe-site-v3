...
duplicates.forEach(([id, files]) => {
  ...
  files.forEach(f => console.log(`  - ${f.name} (${path.basename(f.file)})`));
});
console.log('');
...
// 7. 確認表示（最初の20件のみ）
updates.slice(0, 20).forEach(({ file, name, oldId, newId }) => {
  console.log(`${name}: ID ${oldId} → ${newId} (${path.basename(file)})`);
});
...
console.log(`node fix_duplicate_ids_execute.cjs\n`);
